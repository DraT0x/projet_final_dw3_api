// Erreurs et warnings du es.lint ont été réglé par Claude.AI -- 2025-11-24

import insertUrlParams from 'inserturlparams';
import { customDeepCompare } from 'jet-validators/utils';
import mockMongoose from '@jazim/mock-mongoose';
...
const compareUserArrays = customDeepCompare({
  onlyCompareProps: ['titre', 'artiste', 'chansons', 'genres', 'possession'],
});

// Type helper for mockify
const mockify = mockMongoose as (model: typeof Vinyle) => {
  toReturn: (data: unknown, method: string) => {
    toReturn: (data: unknown, method: string) => void,
  },
};
